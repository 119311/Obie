...
def ListUpHistory(winManager):
    ...
    fontTitle = tkFont.Font(regWin, family="Helve", size=30)
    fontLabel = tkFont.Font(regWin, family="Helve", size=12)
    ...
    def Reload(gList=rList[0]):
        ...
        for index, item in enumerate(rList[0]):
            tree.insert(
                parent="", index="end", iid=index, values=tuple(item), tags=item[5]
            )
    ...
    def ItemModify(event):
        def ModifyWindow(record_values, item):
            ...
            # footer
            foot = tk.Canvas(regMod, width=660, height=footSize, bg=appColors["blue"])
            foot.place(x=-10, y=(480 - footSize))

            dt = datetime.datetime.now()

            fontTitle = tkFont.Font(regMod, family="Helve", size=30)
            fontLabel = tkFont.Font(regMod, family="Helve", size=12)
            ...
            def SwitchSS(ssMode, ssModeStr, regMod):
                ...
                titleName = "支出" if ssMode[0] else "収入"
                resultTitile = titleName + "データ確認・編集：Obie"
            ...
            iYear = tk.Entry(
                regMod, textvariable=year, font=fontLabel, width=widthLabel
            )
            ...
            iMonth = tk.Entry(
                regMod, textvariable=month, font=fontLabel, width=widthLabel
            )
            ...
            iDate = tk.Entry(regMod, textvariable=day, font=fontLabel, width=widthLabel)
            ...
            iUse = tk.Entry(regMod, textvariable=use, font=fontLabel, width=widthLabel)
            ...
            iTarget = tk.Entry(
                regMod, textvariable=target, font=fontLabel, width=widthLabel
            )
            ...
            iCost = tk.Entry(
                regMod, textvariable=money, font=fontLabel, width=widthLabel
            )
            ...
            iComment = tk.Entry(
                regMod, textvariable=comment, font=fontLabel, width=widthLabel
            )
    ...
    # ラベル
    menuTitle = tk.Label(
        regWin, text="履歴", fg="black", bg=appColors["lWhite"], font=fontTitle
    )
    ...
    style.configure(
        "Treeview.Heading",
        rowheight=100,
        foreground=appColors["letter"],
        background="white",
    )
    # style.map('Treeview', foreground=fixed_map('foreground'), background=fixed_map('background'))
    tree.tag_configure(
        "収入", foreground=appColors["syunyu"], background=appColors["syunyuB"]
    )
    tree.tag_configure(
        "支出", foreground=appColors["sisyutu"], background=appColors["sisyutuB"]
    )
    ...
    def Serch():
        def DataSerch(year, month, date, use, target, costu, costl, comment):
            csvList, file_path = importcsv.inputlist()
            if year != "":
                csvList = importcsv.csvSort(csvList, 0, int(year))
            if month != "":
                csvList = importcsv.csvSort(csvList, 1, int(month))
            if date != "":
                csvList = importcsv.csvSort(csvList, 2, int(date))
            if use != "":
                csvList = importcsv.csvSort(csvList, 3, use)
            if target != "":
                csvList = importcsv.csvSort(csvList, 6, target)
            if comment != "":
                csvList = importcsv.csvSort(csvList, 7, comment)

            if costu != "":
                csvList = importcsv.csvmoneySort(csvList, int(costu), False)
            if costl != "":
                csvList = importcsv.csvmoneySort(csvList, int(costl), True)
        ...
        # ロゴ表示
        titleCanvas = tk.Canvas(regMod, width=640, height=40, bg=appColors["banner"])
        titleCanvas.place(x=0, y=0)
        ...
        titleName = ""
        resultTitile = titleName + "検索：Obie"
        regMod.title(resultTitile)

        # ラベル
        menuTitle = tk.Label(
            regMod, text="検索", fg="black", bg=appColors["lWhite"], font=fontTitle
        )
        ...
        iYear = tk.Entry(regMod, textvariable=year, font=fontLabel, width=widthLabel)
        iYear.place(x=150, y=120)
        ...
        iMonth = tk.Entry(regMod, textvariable=month, font=fontLabel, width=widthLabel)
        iMonth.place(x=150, y=145)
        ...
        iCostu = tk.Entry(regMod, textvariable=moneyu, font=fontLabel, width=widthLabel)
        ...
        iCostl = tk.Entry(regMod, textvariable=moneyl, font=fontLabel, width=widthLabel)

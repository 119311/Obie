...
def RegisterData(winManager):
    ...
    # ロゴ表示
    titleCanvas = tk.Canvas(regWin, width=640, height=40, bg=appColors["banner"])
    ...
    def SwitchSS(ssMode, ssModeStr, regWin):
        ...
        resultTitile = titleName + "データ登録：Obie"
    ...
    # ラベル
    menuTitle = tk.Label(
        regWin, text="データ登録", fg="black", bg=appColors["lWhite"], font=fontTitle
    )
    menuTitle.place(x=230, y=50)

    lYear = tk.Label(
        regWin, text="年", fg=appColors["letter"], bg=appColors["lWhite"], font=fontLabel
    )

    lYear.place(x=100, y=120)

    iYear = tk.Entry(regWin, textvariable=year, font=fontLabel, width=widthLabel)
    iYear.place(x=150, y=120)
    iYear.insert(tk.END, str(dt.year))

    lMonth = tk.Label(
        regWin, text="月", fg=appColors["letter"], bg=appColors["lWhite"], font=fontLabel
    )

    lMonth.place(x=100, y=145)

    iMonth = tk.Entry(regWin, textvariable=month, font=fontLabel, width=widthLabel)
    iMonth.place(x=150, y=145)
    iMonth.insert(tk.END, str(dt.month))

    lDate = tk.Label(
        regWin, text="日", fg=appColors["letter"], bg=appColors["lWhite"], font=fontLabel
    )

    lDate.place(x=100, y=170)

    iDate = tk.Entry(regWin, textvariable=day, font=fontLabel, width=widthLabel)
    ...
    iUse = tk.Entry(regWin, textvariable=use, font=fontLabel, width=widthLabel)
    ...
    iTarget = tk.Entry(regWin, textvariable=target, font=fontLabel, width=widthLabel)
    ...
    iCost = tk.Entry(regWin, textvariable=money, font=fontLabel, width=widthLabel)
    ...
    iComment = tk.Entry(regWin, textvariable=comment, font=fontLabel, width=widthLabel)
